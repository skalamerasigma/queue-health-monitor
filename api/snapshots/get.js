...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    try {
      ...
      const { tseIds, startDate, endDate } = req.query;
      
      let result;
      
      if (startDate && endDate) {
        result = await db.query(`
          SELECT date, timestamp, tse_data
          FROM tse_snapshots
      ...
      // Transform results
      let snapshots = result.rows.map(row => ({
        date: row.date,
        timestamp: row.timestamp,
        tseData: row.tse_data
      }));

      // Filter by TSE IDs if provided
      if (tseIds) {
        const tseIdArray = Array.isArray(tseIds) ? tseIds : [tseIds];
        snapshots = snapshots.map(snapshot => ({
          ...snapshot,
          tseData: snapshot.tseData.filter(tse => tseIdArray.includes(String(tse.id)))
